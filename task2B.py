...
type_dict = {all_types_data['results'][index]['name']:index for index in range(18)}

damage_multipliers = [['1' for i in range(18)] for i in range(18)]

def fetch_type_data(defender_name):
    global damage_multipliers, type_dict
    ...
    row_index = type_dict[defender_name]

    for attacker_name in double_damage_from:
        col_index = type_dict[attacker_name]
...
with ThreadPoolExecutor(max_workers=5) as executor:
    results = executor.map(fetch_type_data, type_dict.keys())
